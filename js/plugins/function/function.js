/**
 * Function 插件 - 自定义函数可视化
 * 
 * 特点：
 * - 矩形视窗框架，(0,0) 在中心
 * - 支持用户自定义数学函数表达式（使用 math.js）
 * - 支持 x/y 轴独立缩放倍数
 * - 视窗透明无背景，只显示函数曲线
 */
...
// 默认配置（集中管理）
// ═══════════════════════════════════════════
const FUNCTION_DEFAULT_SETTINGS = {
    expression: '(4/pi) * (sin(x) + (1/3)*sin(3*x) + (1/5)*sin(5*x) + (1/7)*sin(7*x))',
    width: 4,
    height: 4,
    x_scale: 1,
    y_scale: 1,
...
const FUNCTION_PREVIEW_STYLES = {
    viewport_color: 'rgba(52, 152, 219, 0.3)',
...
// 辅助函数：安全计算函数表达式
function evaluateFunctionSafe(expr, x) {
    try {
        // 使用 math.js（如果可用）
...
registerShape({
    ...
    createDefault: function(x, y) {
        return {
            type: 'function',
            name: 'function_' + (ManimEditor.elements.length + 1),
            props: {
                x: x !== undefined ? x : 0,
                y: y !== undefined ? y : 0,
                width: 0,   // 视窗宽度（Manim单位），从0开始避免闪现
                height: 0,  // 视窗高度（Manim单位），从0开始避免闪现
                expression: FUNCTION_DEFAULT_SETTINGS.expression,
                x_scale: FUNCTION_DEFAULT_SETTINGS.x_scale,
                y_scale: FUNCTION_DEFAULT_SETTINGS.y_scale,
                color: FUNCTION_DEFAULT_SETTINGS.color,
                stroke_width: FUNCTION_DEFAULT_SETTINGS.stroke_width,
                z_order: FUNCTION_DEFAULT_SETTINGS.z_order,
                hidden: false
            }
        };
    },
    ...
    render: function(ctx, element, editor) {
        ...
        const width = props.width !== undefined ? props.width : FUNCTION_DEFAULT_SETTINGS.width;
        const height = props.height !== undefined ? props.height : FUNCTION_DEFAULT_SETTINGS.height;
        const expression = props.expression !== undefined ? props.expression : FUNCTION_DEFAULT_SETTINGS.expression;
        const xScale = props.x_scale !== undefined ? props.x_scale : FUNCTION_DEFAULT_SETTINGS.x_scale;
        const yScale = props.y_scale !== undefined ? props.y_scale : FUNCTION_DEFAULT_SETTINGS.y_scale;
        
        // 动态计算采样点数：确保曲线平滑
        // 每个 Manim 单位至少 100 个采样点，最少 300，最多 1000
        const xRangeManim = width * xScale;
        const optimalSamples = Math.max(300, Math.floor(xRangeManim * 100));
        const samples = Math.min(optimalSamples, 1000);
        ...
        if (ManimEditor.selectedElement?.id === element.id) {
            ctx.strokeStyle = FUNCTION_PREVIEW_STYLES.viewport_color;
            ctx.lineWidth = FUNCTION_PREVIEW_STYLES.viewport_line_width;
            ctx.setLineDash(FUNCTION_PREVIEW_STYLES.viewport_dash);
            ctx.strokeRect(center.x - w / 2, center.y - h / 2, w, h);
            ctx.setLineDash([]);
        }
        
        // 绘制函数曲线
        ctx.strokeStyle = props.color !== undefined ? props.color : FUNCTION_DEFAULT_SETTINGS.color;
        ctx.lineWidth = props.stroke_width !== undefined ? props.stroke_width : FUNCTION_DEFAULT_SETTINGS.stroke_width;
        ctx.lineJoin = 'round';
        ctx.lineCap = 'round';
        
        // 函数 x 范围：相对于视窗中心(0,0)的局部坐标系
        // 视窗左边界 x = -width/2，右边界 x = width/2
        // 应用 xScale 后：函数 x 从 -width/2*xScale 到 width/2*xScale
        const xFuncMin = -(width / 2) * xScale;
        const xFuncMax = (width / 2) * xScale;
        ...
        for (let i = 0; i <= samples; i++) {
            const t = i / samples;
            const xFunc = xFuncMin + (xFuncMax - xFuncMin) * t;  // 函数输入的 x（视窗局部坐标）
            const yFunc = evaluateFunctionSafe(expression, xFunc);  // 函数输出的 y（视窗局部坐标）
            
            // 将函数坐标（视窗局部）映射到场景坐标（Manim全局）
            const xGlobal = centerX + xFunc / xScale;  // 场景坐标
            const yGlobal = centerY + yFunc / yScale;  // 场景坐标
        ...
        // 绘制中心点标记
        ctx.fillStyle = FUNCTION_PREVIEW_STYLES.center_dot_color;
        ctx.globalAlpha = FUNCTION_PREVIEW_STYLES.center_dot_alpha;
        ctx.beginPath();
        ctx.arc(center.x, center.y, FUNCTION_PREVIEW_STYLES.center_dot_radius, 0, Math.PI * 2);
        ctx.fill();
    },
    ...
    hitTest: function(element, manimX, manimY, editor) {
        const props = element.props;
        const halfWidth = (props.width !== undefined ? props.width : FUNCTION_DEFAULT_SETTINGS.width) / 2;
        const halfHeight = (props.height !== undefined ? props.height : FUNCTION_DEFAULT_SETTINGS.height) / 2;
    ...
    handleScale: function(element, scaleInfo, editor) {
        ...
        // 等比例
        if (isShift) {
            const origW = originalProps.width !== undefined ? originalProps.width : FUNCTION_DEFAULT_SETTINGS.width;
            const origH = originalProps.height !== undefined ? originalProps.height : FUNCTION_DEFAULT_SETTINGS.height;
    ...
    toManim: function(element) {
        ...
        const color = hexToManimColor(props.color !== undefined ? props.color : FUNCTION_DEFAULT_SETTINGS.color);
        const expression = props.expression !== undefined ? props.expression : FUNCTION_DEFAULT_SETTINGS.expression;
        const xScale = props.x_scale !== undefined ? props.x_scale : FUNCTION_DEFAULT_SETTINGS.x_scale;
        const yScale = props.y_scale !== undefined ? props.y_scale : FUNCTION_DEFAULT_SETTINGS.y_scale;
        ...
        code += `lambda x: (${pythonExpr}) / ${formatNumber(yScale)}, `;
        code += `x_range=[${formatNumber(xFuncMin)}, ${formatNumber(xFuncMax)}], `;
        code += `color=${color}`;
        
        const strokeWidth = props.stroke_width !== undefined ? props.stroke_width : FUNCTION_DEFAULT_SETTINGS.stroke_width;
        if (strokeWidth !== FUNCTION_DEFAULT_SETTINGS.stroke_width) {
    ...
    properties: [
        { key: 'expression', label: '函数表达式', type: 'text' },
