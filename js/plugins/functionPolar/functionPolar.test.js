...
global.math = {
    evaluate: function(expr, scope) {
        ...
        // 简单模拟常见极坐标函数
        if (expr.includes('sin') && expr.includes('3')) {
            return Math.sin(3 * theta);
        }
...
// 加载插件
require('./functionPolar.js');

const plugin = ManimEditor.shapeRegistry['functionPolar'];
let passed = 0;
let failed = 0;
...
console.log('🧪 FunctionPolar 极坐标函数插件测试\n');
...
ok(el.type === 'functionPolar', 'createDefault 返回 functionPolar');
ok(el.props.expression === 'sin(3 * theta)', '默认表达式正确');
ok(el.props.width === 0, '初始宽度为0');
ok(el.props.height === 0, '初始高度为0');
ok(el.props.theta_max === 2 * Math.PI, '默认 theta_max 为 2π');
...
ok(code.includes('ParametricFunction'), 'toManim 包含 ParametricFunction');
ok(code.includes('lambda'), 'toManim 包含 lambda');
ok(code.includes('np.sin') && code.includes('np.cos'), 'toManim 转换三角函数');
ok(code.includes('t_range=[0, 12.57]'), 'toManim 包含 theta 范围 (4π ≈ 12.57)');
ok(code.includes('/ 2'), 'toManim 应用 y_scale');

// 测试9: properties
ok(plugin.properties.length >= 9, 'properties 包含所有属性');
